			...
			if (likedUser.likes.includes(currentUser.id)) {
				...
				if (currentSocketId) {
					io.to(currentSocketId).emit("newMatch", {
						...
						university: likedUser.university,
						course: likedUser.course
					});
				}
			}
		}
...
export const getMatches = async (req, res) => {
	try {
		const user = await User.findById(req.user.id).populate("matches", "name image university course");
	...
	try {
		...
		const users = await User.find({
			$and: [
				...
				// Optionally add university filtering here if needed
				// { university: currentUser.university } 
			],
		}).select("name image age gender university course");
